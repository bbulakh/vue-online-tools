<template>
  <div class="relative px-[2rem] overflow-hidden mb-10">
    <img
      class="w-[280px] left-[150px] top-[25px] z-[-1] absolute opacity-75"
      src="/src/assets/images/bg-doodle.svg"
      alt="Background doodle"
    />
    <h1 class="text-8xl mb-1">Cools</h1>
    <h3 class="mb-5 text-lg">
      Collection of useful and fun tools that you can use to work with numbers, strings and more
    </h3>
    <div class="flex flex-col gap-3 justify-center">
      <button class="bg-[#A8EB53] py-3 rounded-md">EXPLORE</button>
      <button
        class="bg-[#242424] text-white py-3 rounded-md flex justify-center items-center gap-2"
      >
        <svg xmlns="http://www.w3.org/2000/svg" width="15" height="14" viewBox="0 0 16 16">
          <path
            fill="currentColor"
            d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59c.4.07.55-.17.55-.38c0-.19-.01-.82-.01-1.49c-2.01.37-2.53-.49-2.69-.94c-.09-.23-.48-.94-.82-1.13c-.28-.15-.68-.52-.01-.53c.63-.01 1.08.58 1.23.82c.72 1.21 1.87.87 2.33.66c.07-.52.28-.87.51-1.07c-1.78-.2-3.64-.89-3.64-3.95c0-.87.31-1.59.82-2.15c-.08-.2-.36-1.02.08-2.12c0 0 .67-.21 2.2.82c.64-.18 1.32-.27 2-.27c.68 0 1.36.09 2 .27c1.53-1.04 2.2-.82 2.2-.82c.44 1.1.16 1.92.08 2.12c.51.56.82 1.27.82 2.15c0 3.07-1.87 3.75-3.65 3.95c.29.25.54.73.54 1.48c0 1.07-.01 1.93-.01 2.2c0 .21.15.46.55.38A8.012 8.012 0 0 0 16 8c0-4.42-3.58-8-8-8"
          />
        </svg>
        VISIT GITHUB
      </button>
    </div>
  </div>
</template>

<style>
button {
  font-family: 'Koulen';
}
</style>
